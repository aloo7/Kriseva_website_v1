...
|---|---|---|---|---|---|---|---|---|---|
| 1 | Company positioning | `index.astro`, `site.ts` (not currently rendered — superseded by companyV2.sentence, D-006 homepage shrink) | "KRISEVA AI builds procurement intelligence systems for India's defense ecosystem." | unchanged | Verified and safe | `project/uploads/EXECUTIVE_SUMMARY.md` | Low | Retained | No |
| 2 | Hero thesis | `index.astro` (not currently rendered — superseded by companyV2.hero-headline, D-006 homepage shrink) | "A tender is not a PDF. It is a decision system hidden inside a document bundle." | unchanged | Verified and safe | `project/uploads/EXECUTIVE_SUMMARY.md`; `project/uploads/suggested_copy_blocks.md` | Low | Retained | No |
| 3 | Bidder-MSME positioning | `index.astro`, `tas.astro` (not currently rendered — superseded by portfolioV2.tas-primary-user, D-006 homepage shrink) | "KRISEVA TAS helps defense MSMEs and defense-tech teams move from tender discovery to structured bid review." | unchanged | Verified and safe | `project/uploads/CLAIM_REGISTER.md` (row 1); `project/_dev/QA_REPORT.md` §4 | Low | Retained | No |
| 4 | TAS one-liner | `index.astro`, `tas.astro`, `claims.productOneLiner` (not currently rendered — superseded by tas.long-line / portfolioV2.tas-purpose, D-006 homepage shrink) | "KRISEVA TAS helps bid teams discover, parse, score, and review tender opportunities." | unchanged | Verified and safe | `project/uploads/CLAIM_REGISTER.md` (row 1) | Low | Retained | No |
...
| 20 | Issuer-side discovery | `issuer-roadmap.astro` (not currently rendered — retired route, archived as `_issuer-roadmap_archive.astro`) | "The need has been validated through senior procurement-side discovery conversations. Names withheld where confidentiality applies." | unchanged | Safe with care | Stage 6 brief — issuer copy rules; `project/uploads/RESEARCH_LIMITATIONS.md` | Medium | Retained — caveat: anonymized unless explicit written/public permission is on record | No |
...
| 39 | Evaluator eval-status (`evaluator.eval-status`, primary row added Gate 3) | `evaluator.astro`, `evidence.astro` | "In active evaluation with a central armed police force." | unchanged | Safe with care | Founder ruling 2026-06-09 (two co-equal products); `notes/founder-log/FOUNDER_LOG.md` | Medium | Gate 3 correction 2026-08-15 (docs/v2-handoffs/INDEPENDENT_REVIEW_V2.md Major 2): this id was reused as evidence by V2-12 and V2-25 but never had its own primary row; `publicClaims.ts`'s `surface` field also still listed the pre-v2 `index.astro` "cred band" (removed in the v2 homepage rebuild) and `validation.astro` (now archived `_validation_archive.astro`, not a live route). Control-plane ruling: exactly once on `/evaluator`, exactly once on `/evidence`, never on `/company`. Verified by grep that `/company` does not call this id; it renders `evaluatorV2.maturity-working-prototype` instead, which does not carry this sentence. Both fields corrected to `evaluator.astro, evidence.astro`; no wording change | No |
...
| V2-25 | `portfolioV2.evaluator-maturity` | PortfolioCards (not currently rendered — renders instead as its own atomic sub-claims, `evaluatorV2.maturity-working-prototype` + `evaluator.eval-status`; not a violation) | "Working prototype. In active evaluation with a central armed police force." | Safe with care | Architecture §7; reuses `evaluator.eval-status` | Medium (no end-to-end framing here, see evaluatorV2 PENDING-FABLE) | No |
...
| V2-31 | `portfolioV2.attest-maturity` | PortfolioCards, /attest, /finance (not currently rendered — renders instead as its own atomic sub-claims, `attestV2.maturity-research-stage` + `-synthetic-data` + `-no-regulator-connection` + `-no-customers`; not a violation) | "Research-stage, human-reviewed prototype. Synthetic demonstration data. Not connected to any regulator system. No customers, no pilots." | Verified and safe | attest-public-export/README.md (lines 1, 5, 40-41); 00_CONTROL/STATUS.md (lines 26-27) | Low (mandatory §11 statement) | No |
...
| # | Claim id | Page / section | Approved wording | Category | Evidence | Risk | Human approval? |
|---|---|---|---|---|---|---|---|
| V2-44 | `evaluatorV2.functional-verified` | /evaluator, /capabilities (not currently rendered in any form) | "The core evaluation components of EVALUATOR, document ingestion, the hash-chained audit log, the pattern-intelligence layer, the committee-triggered external-verification stack, and the recommendation memorandum generator, are built and functionally verified." | Safe with care | evaluator README (§9, Functional components verified) | Medium (component-level only) | No |
| V2-45 | `evaluatorV2.roadmap-boundary` | /evaluator (boundaries) (not currently rendered — /evaluator's boundaries section uses `boundary.notClaimed` instead) | "Multi-tender breadth, cross-tender bidder identity, and production authentication are wired or planned, not yet demonstrated end to end." | Verified and safe | evaluator README (§9 wired, §9 PLANNED) | Low (self-limiting) | No |
...
### Gate 1 correction: surface-field sync (2026-08-15)

Mechanical sync, no wording changes. `docs/v2-handoffs/CLAIM_AUDIT_V2.md` §1(c) found ~23 claim/page combinations where a claim renders (exact registered wording, confirmed at `getClaim()` call sites) on a live route its `surface` field in `publicClaims.ts` did not list: 11 on `/company` (`act2.two-rooms`, `act7.mission`, `trustV2.human-in-command`, `trustV2.local-first`, `portfolioV2.tas-maturity`, `evaluatorV2.maturity-working-prototype`, `attestV2.maturity-research-stage`, `record.pan-iit`, `record.udyam`, `record.dpiit`, `record.iiec`), 6 on `/capabilities` (`attestV2.source-sealing`, `evaluator.three-path`, `evaluator.six-state`, `attestV2.human-review`, `attestV2.non-interpretive`, `evaluator.audit-chain`), 4 on `/evidence` (`attestV2.demo-hub`, `attestV2.maturity-synthetic-data`, `attestV2.maturity-no-regulator-connection`, `attestV2.maturity-no-customers`), `portfolioV2.attest-purpose` on `/attest` + `/finance` + `/capabilities`, and `founder.education` on `/contact`. Every `surface` field above now lists every route where its claim actually renders. Also registers `validation.screenshot-caption` (wording "Repo-owned capture · seeded demo data", restoring row 26's approved wording to code with a stable id; see CLAIM_AUDIT_V2.md §1(b) finding 2) so every tas.astro screenshot caption renders one locked, claim-sourced string instead of the ~9 ad-hoc variants the audit found. `validation.synthetic-data`'s own wording is untouched.
